...
ORG_PATH=$SUPER_ORG_PATH
BASIC_AUTH=$SUPER_BASIC_AUTH

# Accept optional port as first argument, default to 9443
PORT=${1:-9443}

USERS_URL="https://localhost:$PORT/$ORG_PATH/scim2/Users"
DELETE_URL="https://localhost:$PORT/$ORG_PATH/scim2/Bulk"
...
while true; do
    ...
    if [[ -z "$USER_IDS" || "$USER_IDS" == "null" ]]; then
        echo "No more users left. Total deleted: $TOTAL_DELETED"
        break
    fi

    USER_COUNT=$(echo "$USER_IDS" | wc -l | tr -d ' ')
    echo "Found $USER_COUNT user(s) to delete..."
...
EOL
    ...
    TOTAL_DELETED=$((TOTAL_DELETED + USER_COUNT))
    echo "Deleted $USER_COUNT user(s). Total so far: $TOTAL_DELETED"
